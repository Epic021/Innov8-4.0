...
def parse_apt(s):
    """aptitude_score is out of 10, but recruiters also typed it as a percentage ('69%' -> 6.9)
    or bare 0-100 ('83' -> 8.3); num() left those as 69/83. Detect the %/0-100 scale and /10.
    (463 test rows carry '%'; mis-scaled aptitude also hits 7 dev winners.) Confirmed by the
    audit workbook's build_structured parser."""
    if pd.isna(s):
        return np.nan
    t = str(s).lower().strip()
    x = num(t)
    if np.isnan(x):
        return np.nan
    return x / 10.0 if ("%" in t or x > 10.0) else x


_RATING_WORDS = {"outstanding": 5.0, "exceeds expectations": 4.0, "exceeds": 4.0,
                 "meets expectations": 3.0, "meets": 3.0, "needs improvement": 2.0,
                 "unsatisfactory": 1.0, "new joiner - not rated": np.nan, "not rated": np.nan}


def parse_rating(s):
    """last_rating on a 1-5 scale, typed as a number ('4/5','3.0') OR a word
    ('Outstanding'=5 ... 'Unsatisfactory'=1). The old parser dropped every word rating as
    missing -- 16% of rows, on a top-3 signal. Word map from the audit workbook's dictionary."""
    if pd.isna(s):
        return np.nan
    t = str(s).lower().strip()
    for w, v in _RATING_WORDS.items():
        if w in t:
            return v
    return num(t)
...
NUMERIC = ["c_tech", "c_apt", "c_rating", "c_kpi", "c_exp", "c_age", "c_gy", "c_trn", "c_trh", "c_awards",
           ...
           "c_ctc", "c_ectc", "c_ctc_ratio", "c_nd"]     # c_ectc: expected-CTC LEVEL (was only a ratio)
...
class FeatureBuilder:
    ...
    def fit(self, tr, others):
        ...
        # major and company_type are job-relevant but NOT in the debrief's pet-preference list
        # (college / metro / referral / employer size / degree / gap only), so kept as ordinary
        # predictive features -- never neutralised. One-hot the frequent values.
        self.majors = [m for m, n in allf.major.fillna("NA").str.strip().value_counts().items() if n >= 50]
        self.ctypes = [c for c, n in allf.company_type.fillna("NA").str.strip().value_counts().items() if n >= 50]
    ...
    def transform(self, d):
        ...
        # major / company_type one-hots (job-relevant, non-pedigree -> kept in every regime)
        mj = d.major.fillna("NA").str.strip()
        for m in self.majors:
            cols["major_" + re.sub(r"[^a-z0-9]", "_", m.lower())] = (mj == m).astype(float)
        ct = d.company_type.fillna("NA").str.strip()
        for c in self.ctypes:
            cols["ctype_" + re.sub(r"[^a-z0-9]", "_", c.lower())] = (ct == c).astype(float)
